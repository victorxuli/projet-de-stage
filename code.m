...
% Last Modified by GUIDE v2.5 13-Jul-2018 10:45:19
...
set(handles.code_Status,'String',num2cell(double_tous_les_codes_status_sans_identique));
...
% so window can get raised using code.
...
global S;
[an,~] = find(code==double_tous_les_codes_status_sans_identique);
k = [];
for i = 1:rang_nb_turbine
    k = [k;S.turbine1(i).occurence(an+1,2:end)];
end
imagesc(k);
...
% --- Executes on selection change in code_Status.
function code_Status_Callback(hObject, eventdata, handles)
% hObject    handle to code_Status (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = get(hObject,'String') returns code_Status contents as cell array
%        contents{get(hObject,'Value')} returns selected item from code_Status
global code;
list=get(handles.code_Status,'String');
val1 = get(handles.code_Status,'Value');
code = str2num(list{val1});

% --- Executes during object creation, after setting all properties.
function code_Status_CreateFcn(hObject, eventdata, handles)
% hObject    handle to code_Status (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
